$.extend(window.Parsley.options, {
    ...
    errorTemplate: "<small class='m-1 text-danger'></small>",
})
...
Parsley.on("field:validated", function (el) {
    ...
    if (elNode && !elNode.isValid()) {
        ...
        if (rqeuiredValResult.length > 0) {
            ...
            var formGroupNode = fieldNode.closest(".form-floating")
            var lblNode = formGroupNode.find("label:first")
            if (lblNode.length > 0) {
                // change default error message to include field label
                var errorNode = formGroupNode.find(
                    "div.parsley-error small[class*=parsley-]"
                )
